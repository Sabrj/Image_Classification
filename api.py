...
def received_data():
    ...
    classname = get_class(prediction) ## or just getattr(sys.modules[__name__], prediction# )
    calories = classname.calories
    carbs = classname.carbs
    protein = classname.protein


    return render_template('success.html', img_path=img_path, predict=prediction, calories=calories,carbs=carbs, protein=protein )
